...
exports.getOrder = (req, res, next) => {
    ...
    order_1.OrderModel.findById(id, (err, order) => {
        if (!order) {
            logger_1.OrderAPILogger.logger.info(`[GET] [${req.path}:{orderId}] Order ${id} not found.`);
            return next(new Error(`Order ${id} not found.`));
        }
        order = halson(order.toJSON()).addLink('self', `/store/orders/${order.id}`);
        return utility_1.formatOutput(res, order, 200, 'order');
    });
};
exports.getAllOrders = (req, res, next) => {
    ...
    order_1.OrderModel.find({}, null, { skip: offset, limit: limit }).then(orders => {
        ...
        return utility_1.formatOutput(res, orders, 200, 'order');
    });
};
exports.addOrder = (req, res, next) => {
    ...
    user_1.UserModel.findById(userId, (err, user) => {
        if (!user) {
            logger_1.OrderAPILogger.logger.info(`[POST] [/store/orders/] There is no user with the userId ${userId}`);
        ...
        newOrder.save((error, order) => {
            ...
            return utility_1.formatOutput(res, order, 201, 'order');
        });
    });
};
...
exports.getInventory = (req, res, next) => {
    ...
    order_1.OrderModel.find({ status: status }, (err, orders) => {
        ...
        return utility_1.formatOutput(res, orders, 200, 'inventory');
    });
};
