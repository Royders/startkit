...
export let addUser = (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    OrderAPILogger.logger.error(`[POST] [/users] something went wrong when saveing a new user ${newUser.username} # ${err.message}`)
    next(err)
  }
  newUser.save((error, user) => {
    if (error) {
      OrderAPILogger.logger.error(`[POST] [/users] something went wrong when saveing a new user ${newUser.username} | ${error.message}`)
