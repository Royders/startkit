...
exports.getUser = (req, res, next) => {
    ...
    user_1.UserModel.findOne({ username: username }, (err, user) => {
        ...
        return utility_1.formatOutput(res, user, 200, 'user');
    });
};
exports.addUser = (req, res, next) => {
    ...
    try {
        ...
        logger_1.OrderAPILogger.logger.error(`[POST] [/users] something went wrong when saveing a new user ${newUser.username} # ${err.message}`);
        next(err);
    }
    newUser.save((error, user) => {
        if (error) {
            logger_1.OrderAPILogger.logger.error(`[POST] [/users] something went wrong when saveing a new user ${newUser.username} | ${error.message}`);
            return res.status(500).send(error);
        }
        user = halson(user.toJSON()).addLink('self', `/users/${user._id}`);
        return utility_1.formatOutput(res, user, 201, 'user');
    });
};
exports.updateUser = (req, res, next) => {
    ...
    user_1.UserModel.findOne({ username: username }, (err, user) => {
        if (!user) {
            logger_1.OrderAPILogger.logger.info(`[PATCH] [/users/:{username}] user with username ${username} not found`);
            return res.status(404).send();
        }
...
exports.login = (req, res, next) => {
    ...
    user_1.UserModel.findOne({ username: username }, (err, user) => {
        if (!user) {
            logger_1.OrderAPILogger.logger.info(`[GET] [/users/login] no user found with the username ${username}`);
            return res.status(404).send();
        }
